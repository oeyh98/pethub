...
public class UserController {
        ...
        @ValidToken
        @GetMapping("/api/user")
        public ResponseEntity<?> getOwnerInfoById() throws Exception {
                Long userId = UserContext.userData.get().getUserId();
                UserLoginResponseDto responseDto = userService.getUserById(userId);
        ...
        public ResponseEntity<Object> logout() {
                ...
                return ResponseEntity.ok()
                                .header("Set-Cookie", "ACCESS_TOKEN=; path=/; max-age=0; expires=0; sameSite=None; secure=true;")
                                .header("Set-Cookie", "REFRESH_TOKEN=; path=/updateToken; max-age=0; expires=0;  sameSite=None; secure=true;")
                                .body(ResponseDto.of(
                                                "로그아웃 성공"));
        }
        ...
        public ResponseEntity<Object> updateAccessToken(@CookieValue(REFRESH_TOKEN) String refreshToken)
                ...
                UserTokenResponseDto token = userService.updateAccessToken(UserContext.userData.get().getUserId(),
                                refreshToken);
                ResponseCookie AccessToken = userService.getAccessTokenCookie(
